...
var isAdminUser = token => {
  return new Promise(resolve => {
    redis.redis_client.hmget(
      variables.KEY_ADMIN_TOKENS,
      [token, token],
...
io.on("connect", socket => {
  ...
  socket.on("getAuctionsBidder", request => {
    ...
    executeInPromise(isBidder, request, result => {
      if (result) {
        var usersRef = fireBase.fireBaseConnection
          .database()
          .ref(variables.KEY_AUCTION_MAIN_TABLE)
          .orderByChild("edate");
        ...
        response.message =
          "Error : Invaild / Un-Authorized User. Only Bidder can access this area.";
  ...
  socket.on("getAuctionsAdmin", request => {
    ...
    executeInPromise(isAdminUser, request, result => {
      if (result) {
        var auctionSubTable = fireBase.fireBaseConnection
          .database()
          .ref(variables.KEY_AUCTION_MAIN_TABLE)
          .orderByChild("edate");
        ...
        response.message =
          "Error : Invaild / Un-Authorized User. Only Admin can access this area.";
        socket.emit("unAuthorizedCallback", response);
        return;
      }
    });
      ...
      if (result) {
        if (request.Bid_amt == 0) {
          ...
          new Promise(function(resolve, reject) {
            var auctionMainDataParticularAuction = fireBase.fireBaseConnection
              .database()
              .ref(
                variables.KEY_AUCTION_MAIN_TABLE + "/" + request.product_code
              );

            var lowestBid = 0;

            auctionMainDataParticularAuction.once(
              "value",
              function(snapshot) {
            ...
            if (request.Bid_amt >= result) {
              ...
              console.log("before");
              response.message =
                "Bid value cannot be greater than or equal to " + result;
              console.log(response.message);
              socket.emit("postBidCallback", response);
              return;
            }

            var addBidToSubTable = fireBase.fireBaseConnection
              .database()
              .ref(
                variables.KEY_AUCTION_SUB_TABLE +
                  "/" +
            ...
            var addBidToMainTable = fireBase.fireBaseConnection
              .database()
              .ref(
                variables.KEY_AUCTION_MAIN_TABLE + "/" + request.product_code
              )
  ...
  socket.on("addNewAuction", request => {
    ...
    executeInPromise(isAdminUser, request, result => {
      if (result) {
        var auctionMainTable = fireBase.fireBaseConnection
          .database()
          .ref(variables.KEY_AUCTION_MAIN_TABLE + "/" + request.productCode);
        console.log(request);
        ...
        response.message =
          "Error : Invaild / Un-Authorized User. Only Admin can add auctions. ";
        socket.emit("unAuthorizedCallback", response);
        return;
      }
    });
  ...
  socket.on("addNewBidder", request => {
    ...
    executeInPromise(isAdminUser, request, result => {
      if (result) {
        ...
        var auctionUserTable = fireBase.fireBaseConnection
          .database()
          .ref(variables.KEY_AUCTION_USER_TABLE + "/" + request.bidder_id);

        if (
          request.bidder_id == 0 ||
          request.bidder_email == 0 ||
          request.bidder_name == 0
        ) {
          console.log("bidder??");
          response.msg = "error";
          response.message = "All inputs are mandatory";
  ...
  socket.on("bidlogForBidder", request => {
    ...
    executeInPromise(isBidder, request, result => {
      if (result) {
        var mainTable = fireBase.fireBaseConnection
          .database()
          .ref(variables.KEY_AUCTION_SUB_TABLE + "/" + request.product_code)
          .orderByChild("user_id")
          .equalTo(request.user_id);
    ...
    executeInPromise(isAdminUser, request, result => {
      if (result) {
        ...
        var auctionSubTable = fireBase.fireBaseConnection
          .database()
          .ref(variables.KEY_AUCTION_SUB_TABLE + "/" + request.product_code)
          .orderByChild("bid_amount");

        auctionSubTable.once(
          "value",
          function(snapshot) {
            var request = {};
            request.logs = [];
        ...
        response.message =
          "Error : Invaild / Un-Authorized User. Only Admin User can access this area.";
        socket.emit("unAuthorizedCallback", response);
        return;
      }
    });
...
function isAdminAdded() {
  redis.redis_client.hmget(variables.KEY_USERS, ["user_id", "admin"], function(
    error,
    result
  ) {
    console.log("2");
  ...
  redis.redis_client.hmget(
    ...
    function(error, result) {
      ...
      if (result[1] == null) {
        ...
        if (
          helper.getMD5Hash(credential.user_password) ==
          dbResponse.user_password
        ) {
          var token = uuidv4();
